...
export default function HomeScreen({ navigation }: any) {
  ...
  const startInstantMeeting = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const id = user.id; // personal meeting like web
  ...
  const scheduleMeeting = useCallback(async () => {
    ...
    try {
      ...
      await call.getOrCreate({
        data: { starts_at, custom: { description: 'Scheduled from mobile' } },
      });
  ...
  const joinMeeting = useCallback(async () => {
    ...
    try {
      ...
      if (calls.length) {
        navigation.navigate('Meeting', { callId: joinId });
      } else {
        Alert.alert('Not found', 'No meeting with that ID');
      }
    } catch (e) {
    ...
    <View style={{ flex: 1, backgroundColor: '#1C1F2E' }}>
      <ImageBackground source={require('../../assets/images/logo.png')} resizeMode="contain" style={{ height: 240, margin: 16, borderRadius: 16, overflow: 'hidden', padding: 16, backgroundColor: '#1F2339' }}>
        <View style={{ flex: 1, justifyContent: 'space-between' }}>
          ...
          <View>
            <Text style={{ color: '#ECF0FF', fontSize: 48, fontFamily: 'Nunito_sans_extrabold' }}>12:30</Text>
      ...
      <View style={{ paddingHorizontal: 16 }}>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>
          <TouchableOpacity onPress={startInstantMeeting} style={{ backgroundColor: '#FF742E', flexGrow: 1, padding: 16, borderRadius: 12, minWidth: '46%' }} disabled={loading}>
            <Text style={{ color: '#FFFFFF', fontFamily: 'Nunito_sans_bold', textTransform: 'capitalize' }}>new meeting</Text>
            <Text style={{ color: '#FFFFFF', opacity: 0.85, fontFamily: 'Nunito_sans_regular' }}>Start an instant meeting</Text>
          </TouchableOpacity>
          <View style={{ backgroundColor: '#0E78F9', flexGrow: 1, padding: 16, borderRadius: 12, minWidth: '46%' }}>
            <Text style={{ color: '#FFFFFF', fontFamily: 'Nunito_sans_bold', textTransform: 'capitalize' }}>join meeting</Text>
            <TextInput value={joinId} onChangeText={setJoinId} placeholder="Enter meeting ID" placeholderTextColor="#C9DDFF" style={{ color: 'white', borderBottomColor: '#ECF0FF', borderBottomWidth: 1, paddingVertical: 6 }} />
          ...
          <TouchableOpacity onPress={scheduleMeeting} style={{ backgroundColor: '#830EF9', flexGrow: 1, padding: 16, borderRadius: 12, minWidth: '46%' }}>
            <Text style={{ color: '#FFFFFF', fontFamily: 'Nunito_sans_bold', textTransform: 'capitalize' }}>schedule meeting</Text>
            <Text style={{ color: '#FFFFFF', opacity: 0.85, fontFamily: 'Nunito_sans_regular' }}>Plan your meeting</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => navigation.navigate('Recordings')} style={{ backgroundColor: '#F9A90E', flexGrow: 1, padding: 16, borderRadius: 12, minWidth: '46%' }}>
            <Text style={{ color: '#1C1F2E', fontFamily: 'Nunito_sans_bold', textTransform: 'capitalize' }}>view recordings</Text>
            <Text style={{ color: '#1C1F2E', opacity: 0.85, fontFamily: 'Nunito_sans_regular' }}>Meeting recordings</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
