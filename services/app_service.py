...
class AppService:
    """top level orchestration for text, ocr, and image flows"""
    ...
    def initialize_services(self) -> None:
        """initialize services and preload data if present"""
        try:
    ...
    def simplify_query(self, query: str) -> str:
        """strip common words, keep key terms"""
    ...
    def process_ocr_query(self, image_file) -> TextQueryResult:
        """save image, extract text, search"""
        try:
    ...
    def process_image_product_search(self, image_file) -> Dict[str, Any]:
        """save product image, try multimodal, then fallback"""
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix='.jpg') as temp_file:
                image_file.save(temp_file.name)
                temp_path = temp_file.name
            try:
                ...
                if re.search(r"\b(MISSING|MIXED\s*UP|UNKNOWN|N/?A|POSTAGE|CARRIAGE|SAMPLE|DAMAGED|BROKEN)\b", query_text, re.IGNORECASE):
                    return {"products": [], "response": "No products found.", "predicted_class": predicted_class}
                ...
                if products:
                    ...
                    return {"products": formatted_products, "response": "Results:", "predicted_class": predicted_class}
                else:
                    try:
            ...
            return {"products": [], "response": f"An error occurred while processing the product image: {str(e)}", "predicted_class": "Unknown"}